...
export function shouldSendHighRiskAlert(
  score: number,
  threshold: number
): boolean {
  return (
...
function buildAlertText(
  ...
  const heading = isTest
    ? "FraudLens test high-risk alert"
    : "FraudLens high-risk transaction alert";
  ...
  return [
    ...
    isTest
      ? "This is a test alert from the active workspace."
      : "Review the case in FraudLens as soon as possible.",
  ].join("\n");
}
...
export async function sendAlertNotifications(
  ...
  if (
    !options.force &&
    !shouldSendHighRiskAlert(score, preferences.riskThreshold)
  )
    return [];

  const channels = new Set(options.channels ?? ALERT_CHANNELS);
  const text = buildAlertText(transaction, score, Boolean(options.force));
  const subject = options.force
    ? "FraudLens test high-risk alert"
    : `FraudLens alert: ${transaction.reference} scored ${score}/100`;
    ...
    if (!preferences.emailEnabled)
      ...
      deliveries.push(
        Promise.resolve({
          channel: "email",
          status: "skipped",
          reason: "Resend is not configured on the server",
        })
      );
    else
      deliveries.push(
        deliver("email", () =>
          sendResendEmail(preferences.toEmail!, subject, text)
        )
      );
  }
    ...
    if (!preferences.slackEnabled)
      deliveries.push(
        Promise.resolve({
          channel: "slack",
          status: "skipped",
          reason: "Slack alerts are disabled",
        })
      );
      ...
      deliveries.push(
        Promise.resolve({
          channel: "slack",
          status: "skipped",
          reason: "No Slack webhook URL is configured",
        })
      );
    else if (!isAllowedSlackWebhookUrl(preferences.slackWebhookUrl))
      deliveries.push(
        Promise.resolve({
          channel: "slack",
          status: "skipped",
          reason:
            "The stored Slack webhook URL is not an allowed incoming webhook URL",
        })
      );
    else
      deliveries.push(
        deliver("slack", () =>
          postWebhook(preferences.slackWebhookUrl!, { text })
        )
      );
  }
    ...
    if (!preferences.teamsEnabled)
      deliveries.push(
        Promise.resolve({
          channel: "teams",
          status: "skipped",
          reason: "Teams alerts are disabled",
        })
      );
    else if (!preferences.teamsWebhookUrl)
      deliveries.push(
        Promise.resolve({
          channel: "teams",
          status: "skipped",
          reason: "No Teams workflow URL is configured",
        })
      );
    else if (!isAllowedTeamsWebhookUrl(preferences.teamsWebhookUrl))
      deliveries.push(
        Promise.resolve({
          channel: "teams",
          status: "skipped",
          reason: "The stored Teams webhook URL is not an allowed workflow URL",
        })
      );
    else
      deliveries.push(
        deliver("teams", () =>
          postWebhook(preferences.teamsWebhookUrl!, { text })
        )
      );
  }
