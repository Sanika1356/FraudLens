...
import { createTestAlertTransaction, sendAlertNotifications } from "./notifications";
...
describe("high-risk notification delivery", () => {
  ...
  it("delivers enabled Slack and Teams alerts after a score reaches the organization threshold", async () => {
    ...
    await upsertNotificationPreferences(orgId, {
      ...
      teamsEnabled: true,
      teamsWebhookUrl: "https://prod-01.westus.logic.azure.com/workflows/test/triggers/manual/paths/invoke",
      riskThreshold: 80,
    });

    const results = await sendAlertNotifications(orgId, createTestAlertTransaction(), 80);
  ...
  it("does not invoke any notification transport below the organization threshold", async () => {
    ...
    const results = await sendAlertNotifications(orgId, createTestAlertTransaction(), 84);
