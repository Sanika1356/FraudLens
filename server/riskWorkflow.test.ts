...
import {
  applyCaseUpdate,
  applyCaseWorkflowUpdate,
  caseUpdateSchema,
  caseWorkflowUpdateSchema,
  notificationPreferencesSchema,
  reportFiltersSchema,
  riskInputSchema,
  weeklySummaryPreferencesSchema,
} from "./routers";
...
describe("FraudLens workflow validation", () => {
  ...
  it("accepts only defined resolution reason codes", () => {
    expect(
      caseUpdateSchema.safeParse({
        id: 7,
        caseStatus: "confirmed_fraud",
        note: "Validated fraud outcome.",
    ...
    expect(
      caseUpdateSchema.safeParse({
        ...
        caseStatus: "confirmed_fraud",
        note: "Validated fraud outcome.",
        resolutionReasonCode: "unverified",
      }).success
    ).toBe(false);
  });
  ...
  it("sets ownership, priority, and a due date for an active case", () => {
    ...
    const record = {
      ...
      assigneeId: null,
      assigneeName: null,
      casePriority: "standard" as const,
      dueAt: null,
    };
    const dueAt = new Date("2026-08-18T17:00:00.000Z");
    const parsed = caseWorkflowUpdateSchema.parse({
      id: record.id,
      assigneeId: "user_analyst_42",
  ...
  it("parses valid CSV rows and reports malformed row fields without discarding valid rows", () => {
    ...
    expect(parsed.candidates).toEqual([
      expect.objectContaining({
        row: 2,
        reference: "FRAUD-CSV-001",
        input: expect.objectContaining({ amount: 279.99, deviceStatus: "new" }),
      }),
    ]);
  ...
  it("accepts permitted evidence formats and rejects content or extension spoofing", () => {
    const pdf = Buffer.from("%PDF-1.7\nEvidence document", "utf8").toString(
      "base64"
    );
    const valid = decodeAndValidateEvidenceAttachment({
      fileName: "review.pdf",
    ...
    expect(() =>
      decodeAndValidateEvidenceAttachment({
        fileName: "review.pdf",
        mimeType: "image/png",
        contentBase64: pdf,
      })
    ...
    ).toThrow("valid PDF header");
    expect(() =>
      decodeAndValidateEvidenceAttachment({
        fileName: "notes.txt",
        mimeType: "text/plain",
        contentBase64: Buffer.from([0x61, 0x00, 0x62]).toString("base64"),
      })
    ).toThrow("null bytes");
  });
  ...
  it("creates segregated evidence keys without trusting caller file paths", () => {
    expect(
      createEvidenceStorageKey("org_alpha", 42, "merchant invoice.pdf")
    ).toMatch(/^evidence\/org_alpha\/42\/merchant-invoice\.pdf$/);
    expect(
      createEvidenceStorageKey("org_beta", 42, "merchant invoice.pdf")
    ).not.toContain("org_alpha");
  });
  ...
  it("restricts public API keys to transaction submission and rejects revoked or expired keys", () => {
    expect(
      parseApiKeyScopes(JSON.stringify([PUBLIC_API_KEY_SCOPE, "admin:all"]))
    ).toEqual([PUBLIC_API_KEY_SCOPE]);
    expect(parseApiKeyScopes("not-json")).toEqual([]);
    const now = new Date("2026-08-13T12:00:00.000Z");
    expect(isApiKeyActive(null, null, now)).toBe(true);
    expect(
      isApiKeyActive(null, new Date("2026-08-13T12:01:00.000Z"), now)
    ).toBe(true);
    expect(
      isApiKeyActive(null, new Date("2026-08-13T11:59:00.000Z"), now)
    ).toBe(false);
    expect(
      isApiKeyActive(new Date("2026-08-13T11:00:00.000Z"), null, now)
    ).toBe(false);
  });
  ...
  it("requires a valid recipient before enabling automatic weekly summaries", () => {
    const missingRecipient = weeklySummaryPreferencesSchema.safeParse({
      enabled: true,
      toEmail: null,
    });
    const invalidRecipient = weeklySummaryPreferencesSchema.safeParse({
      enabled: true,
      toEmail: "not-an-email",
    });
    const disabledWithoutRecipient = weeklySummaryPreferencesSchema.safeParse({
      enabled: false,
      toEmail: null,
    });
    const enabledWithRecipient = weeklySummaryPreferencesSchema.safeParse({
      enabled: true,
      toEmail: "fraud-operations@example.com",
    });
  ...
  it("derives organization quality metrics only from confirmed outcomes", () => {
    const report = buildModelQualityReport([
      ...
      },
      {
        ...
        actualOutcome: "fraud",
        classification: "true_positive",
        recordedAt: new Date("2026-08-11T11:00:00.000Z"),
      },
      {
        predictedRiskLabel: "high",
      ...
      },
      {
        ...
        recordedAt: new Date("2026-08-12T12:00:00.000Z"),
      },
    ]);
  ...
  it("derives filtered operational metrics, analyst workload, and downloadable report content", () => {
    ...
    const records = [
      ...
      },
      {
        ...
        caseStatus: "legitimate" as const,
        assigneeId: null,
        assigneeName: null,
        casePriority: "standard" as const,
        dueAt: null,
    ...
    const report = buildOperationalReport(
      ...
      [],
      {
        ...
        dateTo: new Date("2026-08-12T23:59:59.000Z"),
      },
  ...
  it("neutralizes spreadsheet formulas in CSV report fields", () => {
    ...
    const report = buildOperationalReport(
      [{ ...first, merchantName: '=HYPERLINK("https://unsafe.example")' }],
      [],
      {}
    );
  ...
  it("reports required headers and malformed quoted CSV data precisely", () => {
    ...
    const unclosedQuote = parseCsvImport(
      'reference,amount,merchantCategory,transactionCountry,accountCountry,deviceStatus,transactionHour,recentTransactionCount\n"FRAUD-CSV-004,20'
    );

    expect(missingHeader.errors).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ row: 1, field: "merchantCategory" }),
      ])
    );
    expect(unclosedQuote.errors).toEqual([
      expect.objectContaining({
        row: 1,
        field: "file",
