...
export function scoreTransaction(input: RiskInput): RiskDecision {
  ...
  if (input.amount >= 1500) {
    ...
    factors.push(
      factor(
        ...
        "Above-usual amount",
        "low",
        "The amount is above the low-risk reference range."
      )
    );
  }
  ...
  if (input.deviceStatus === "new") {
    ...
    factors.push(
      factor(
        ...
        "New device",
        "high",
        "The transaction was initiated from a device not previously recognised."
      )
    );
  }
  ...
  if (input.transactionCountry !== input.accountCountry) {
    ...
    factors.push(
      factor(
        ...
        "Country mismatch",
        "high",
        "The transaction country differs from the account’s reference country."
      )
    );
  }
  ...
  if (input.recentTransactionCount >= 5) {
    ...
    factors.push(
      factor(
        ...
        "medium",
        "More recent transactions were observed than expected for a routine pattern."
      )
    );
  }
  ...
  if (input.transactionHour <= 5 || input.transactionHour >= 23) {
    ...
    factors.push(
      factor(
        ...
        "medium",
        "The transaction occurred during a lower-activity overnight period."
      )
    );
  }
  ...
  if (elevatedCategories.has(normalisedCategory)) {
    ...
    factors.push(
      factor(
        "category_context",
        "Higher-risk merchant context",
        "low",
        "This merchant category receives additional review attention in the demonstration policy."
      )
    );
  }
