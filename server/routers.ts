...
export const appRouter = router({
  ...
  risk: router({
    ...
    detail: organizationProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .query(({ ctx, input }) => getRecord(ctx.orgId, input.id) ?? null),
    ...
    importCsv: organizationManagerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existingReferences = new Set(
          getRecords(ctx.orgId).map(record => record.reference.toUpperCase())
        );
        Array.from(
          await getTransactionReferencesByOrganization(ctx.orgId)
        ).forEach(reference => existingReferences.add(reference));
        ...
        const records = importable.map(candidate =>
          createRiskRecord(candidate.input, candidate.reference)
        );
