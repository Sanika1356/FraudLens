...
export const PUBLIC_API_KEY_SCOPE =
  "transactions:write" as const satisfies ApiKeyScope;
...
export function parseApiKeyScopes(scopesJson: string): ApiKeyScope[] {
  try {
    ...
    return parsed.filter(
      (scope): scope is ApiKeyScope => scope === PUBLIC_API_KEY_SCOPE
    );
  } catch {
    return [];
  }
...
export function extractBearerApiKey(header: string | undefined): string | null {
  ...
  const match = /^Bearer\s+(fl_live_[A-Za-z0-9_-]{32,128})$/i.exec(
    header.trim()
  );
  return match?.[1] ?? null;
}
