...
export function ApiIntegrationsPage() {
  ...
  const createKey = trpc.apiKeys.create.useMutation({
    onSuccess: (result) => {
      ...
      toast.success("API key created", { description: "Copy it now; the secret cannot be shown again." });
    },
    onError: (error) => toast.error("Unable to create API key", { description: error.message }),
  });
  const revokeKey = trpc.apiKeys.revoke.useMutation({
    onSuccess: () => {
      utils.apiKeys.list.invalidate();
      utils.apiKeys.requestLogs.invalidate();
      toast.success("API key revoked", { description: "New requests using this key will be rejected immediately." });
    },
    onError: (error) => toast.error("Unable to revoke API key", { description: error.message }),
  });

  const submitNewKey = () => {
    const trimmedName = name.trim();
    if (trimmedName.length < 3) {
      toast.error("Enter a descriptive key name", { description: "Use at least three characters so managers can identify the integration." });
      return;
    }
  ...
  const copyIssuedSecret = async () => {
    ...
    try {
      ...
      toast.error("Unable to copy automatically", { description: "Select and copy the key manually before closing this dialog." });
    }
  };

  if (user?.role === "analyst") return <Frame><PageTitle eyebrow="Developer integrations" title="Manager access required" /><QueryState state="error" label="Only managers and administrators can create or revoke workspace API keys." /></Frame>;
  if (keys.isLoading || requestLogs.isLoading) return <Frame><PageTitle eyebrow="Developer integrations" title="Public API" /><QueryState state="loading" label="Loading API integration settings…" /></Frame>;
  if (keys.error || requestLogs.error) return <Frame><PageTitle eyebrow="Developer integrations" title="Public API" /><QueryState state="error" label="Unable to load API integration settings. Refresh and try again." /></Frame>;
  ...
  return <Frame><PageTitle eyebrow="Developer integrations" title="Public API"><div className="flex items-center gap-2"><Badge variant="outline" className="border-cyan-300/20 bg-cyan-300/[0.07] px-3 py-1.5 font-mono text-xs text-cyan-200">v1</Badge><Badge variant="outline" className="border-white/10 bg-white/[0.035] px-3 py-1.5 text-xs text-slate-300">60 requests / minute / key</Badge></div></PageTitle>
    <div className="mb-6 rounded-xl border border-amber-300/15 bg-amber-300/[0.055] px-4 py-3 text-sm leading-6 text-amber-100"><KeyRound className="mr-2 inline h-4 w-4 text-amber-200" /><span className="font-semibold">Treat keys like passwords.</span> Each key grants permission to submit risk assessments for this organization. Copy a new key once, store it in the integration’s secret manager, and revoke it immediately if it is exposed.</div>
    <div className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]"><div className="space-y-6"><Panel><div className="flex items-start gap-3"><div className="rounded-xl bg-cyan-300/10 p-2.5 text-cyan-200"><KeyRound className="h-5 w-5" /></div><div><p className="text-sm font-semibold text-slate-100">Create an API key</p><p className="mt-1 text-sm leading-6 text-slate-500">Keys have the <span className="font-mono text-slate-300">transactions:write</span> scope and can be revoked at any time.</p></div></div><div className="mt-6 space-y-4"><div><label htmlFor="api-key-name" className="text-xs font-medium text-slate-400">Key name</label><Input id="api-key-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Production integration" className="mt-2 border-white/10 bg-[#07111e] text-slate-200 placeholder:text-slate-600 focus-visible:ring-cyan-300" /></div><div><label htmlFor="api-key-expiry" className="text-xs font-medium text-slate-400">Expiry date <span className="text-slate-600">(optional)</span></label><Input id="api-key-expiry" type="date" value={expiresOn} min={dateInput(new Date())} onChange={(event) => setExpiresOn(event.target.value)} className="mt-2 border-white/10 bg-[#07111e] text-slate-200 focus-visible:ring-cyan-300" /></div><Button onClick={submitNewKey} disabled={createKey.isPending} className="w-full bg-cyan-300 text-slate-950 hover:bg-cyan-200"><KeyRound className="mr-2 h-4 w-4" />{createKey.isPending ? "Creating secure key…" : "Create API key"}</Button></div></Panel>
      <Panel><Eyebrow>Integration checklist</Eyebrow><ol className="mt-3 space-y-3 text-sm leading-6 text-slate-400"><li><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-cyan-300/10 text-xs font-semibold text-cyan-200">1</span>Create a named key and copy it before closing the confirmation.</li><li><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-cyan-300/10 text-xs font-semibold text-cyan-200">2</span>Store the secret outside source control and send it as a Bearer credential.</li><li><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-cyan-300/10 text-xs font-semibold text-cyan-200">3</span>Monitor request logs and revoke unused or exposed credentials.</li></ol></Panel></div>
      <div className="space-y-6"><Panel><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-semibold text-slate-100">Submit a transaction assessment</p><p className="mt-1 text-sm leading-6 text-slate-500">The versioned endpoint validates each payload, scores it through the same workflow as the dashboard, creates a review case, and evaluates enabled high-risk alerts.</p></div><a href={`${baseUrl}/api/v1/docs`} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center rounded-md border border-white/10 bg-white/[0.035] px-3 text-xs font-semibold text-slate-200 hover:bg-white/[0.08] hover:text-white">Open JSON docs</a></div><div className="mt-5 rounded-xl border border-white/[0.075] bg-[#07111e] p-4"><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">POST endpoint</p><code className="break-all text-sm text-cyan-200">{endpoint}</code></div><pre className="mt-4 overflow-x-auto rounded-xl border border-white/[0.075] bg-[#07111e] p-4 text-xs leading-6 text-slate-300"><code>{sampleRequest}</code></pre><p className="mt-3 text-xs leading-5 text-slate-500">Required fields are <span className="font-mono text-slate-400">amount</span>, <span className="font-mono text-slate-400">merchantCategory</span>, country codes, device status, transaction hour, and recent transaction count. The optional reference prevents duplicate submissions within this workspace.</p></Panel>
        <Panel><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-slate-100">Active and historical keys</p><p className="mt-1 text-xs text-slate-500">Only a masked prefix is retained for display; the key secret is never stored in readable form.</p></div><KeyRound className="h-4 w-4 text-slate-500" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead><tr className="border-b border-white/[0.07] text-[10px] uppercase tracking-[0.13em] text-slate-500"><th className="pb-3 font-medium">Integration</th><th className="pb-3 font-medium">Scope</th><th className="pb-3 font-medium">Last used</th><th className="pb-3 font-medium">Status</th><th className="pb-3 text-right font-medium">Action</th></tr></thead><tbody>{keys.data?.map((key) => { const expired = !!key.expiresAt && new Date(key.expiresAt) <= new Date(); const active = !key.revokedAt && !expired; return <tr key={key.id} className="border-b border-white/[0.045] last:border-0"><td className="py-3.5"><p className="text-sm font-medium text-slate-200">{key.name}</p><p className="mt-1 font-mono text-[11px] text-slate-500">{key.keyPrefix}</p></td><td className="py-3.5 font-mono text-xs text-cyan-200">{key.scopes.join(", ")}</td><td className="py-3.5 text-xs text-slate-400">{key.lastUsedAt ? date(key.lastUsedAt) : "Never"}</td><td className="py-3.5"><Badge variant="outline" className={active ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-200" : "border-slate-300/15 bg-slate-300/[0.07] text-slate-400"}>{key.revokedAt ? "Revoked" : expired ? "Expired" : "Active"}</Badge></td><td className="py-3.5 text-right">{active ? <AlertDialog><AlertDialogTrigger asChild><Button variant="ghost" className="h-8 text-xs text-rose-200 hover:bg-rose-300/10 hover:text-rose-100">Revoke</Button></AlertDialogTrigger><AlertDialogContent className="border-white/10 bg-[#0c1a28] text-slate-100"><AlertDialogHeader><AlertDialogTitle>Revoke {key.name}?</AlertDialogTitle><AlertDialogDescription className="text-slate-400">Applications using this key will immediately lose transaction-submission access. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="border-white/10 bg-white/[0.035] text-slate-200 hover:bg-white/[0.08] hover:text-white">Cancel</AlertDialogCancel><AlertDialogAction onClick={() => revokeKey.mutate({ id: key.id })} className="bg-rose-300 text-rose-950 hover:bg-rose-200">Revoke key</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : <span className="text-xs text-slate-600">—</span>}</td></tr>; })}</tbody></table>{keys.data?.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">No API keys have been created for this workspace.</p> : null}</div></Panel>
        <Panel><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-slate-100">Recent API requests</p><p className="mt-1 text-xs text-slate-500">Latest 50 attempts across this workspace, including rejected requests.</p></div><History className="h-4 w-4 text-slate-500" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead><tr className="border-b border-white/[0.07] text-[10px] uppercase tracking-[0.13em] text-slate-500"><th className="pb-3 font-medium">Time</th><th className="pb-3 font-medium">Request</th><th className="pb-3 font-medium">Reference</th><th className="pb-3 font-medium">Risk</th><th className="pb-3 text-right font-medium">Status</th></tr></thead><tbody>{requestLogs.data?.map((entry) => <tr key={entry.id} className="border-b border-white/[0.045] last:border-0"><td className="py-3.5 text-xs text-slate-400">{date(entry.createdAt)}</td><td className="py-3.5 font-mono text-xs text-slate-300">{entry.requestId}</td><td className="py-3.5 font-mono text-xs text-slate-300">{entry.transactionReference ?? "—"}</td><td className="py-3.5">{entry.riskLevel ? <RiskPill level={entry.riskLevel as RiskLevel} /> : <span className="text-xs text-slate-600">—</span>}</td><td className="py-3.5 text-right"><Badge variant="outline" className={entry.responseStatus >= 400 ? "border-rose-300/20 bg-rose-300/10 text-rose-200" : "border-emerald-300/20 bg-emerald-300/10 text-emerald-200"}>{entry.responseStatus}</Badge></td></tr>)}</tbody></table>{requestLogs.data?.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">No public API requests have been logged yet.</p> : null}</div></Panel></div></div>
    <AlertDialog open={Boolean(issuedSecret)} onOpenChange={(open) => { if (!open) setIssuedSecret(null); }}><AlertDialogContent className="border-cyan-300/20 bg-[#0c1a28] text-slate-100"><AlertDialogHeader><AlertDialogTitle>Copy the new API key now</AlertDialogTitle><AlertDialogDescription className="text-slate-400">{issuedName} is ready. This secret will disappear when you close this dialog and cannot be retrieved later.</AlertDialogDescription></AlertDialogHeader><div className="rounded-xl border border-cyan-300/20 bg-[#07111e] p-3"><code className="break-all text-sm text-cyan-200">{issuedSecret}</code></div><AlertDialogFooter><AlertDialogCancel className="border-white/10 bg-white/[0.035] text-slate-200 hover:bg-white/[0.08] hover:text-white">I stored it securely</AlertDialogCancel><AlertDialogAction onClick={copyIssuedSecret} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">Copy key</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </Frame>;
}
