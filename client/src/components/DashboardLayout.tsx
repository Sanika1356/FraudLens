      ...
      <div className="flex min-h-screen items-center justify-center bg-[#07111e] p-6 text-slate-100">
        <div className="w-full max-w-md rounded-2xl border border-cyan-300/15 bg-slate-950/70 p-9 shadow-2xl shadow-cyan-950/30">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-400">
            Access the analyst console to review risk assessments and manage
            investigation cases.
          </p>
          <Button
            onClick={() => window.location.assign("/sign-in")}
            className="mt-8 w-full bg-cyan-300 text-slate-950 hover:bg-cyan-200"
          >
            Sign in to FraudLens
          </Button>
        </div>
      </div>
    );
...
function DashboardLayoutContent({
  ...
  const activeMenuItem = availableMenuItems.find(
      ...
      <div className="relative" ref={sidebarRef}>
        ...
        >
            ...
            <div className="flex w-full items-center gap-3 px-1">
              ...
              {!isCollapsed && (
                <div className="min-w-0">
                  ...
                  <p className="mt-1 pl-8 text-[10px] font-medium uppercase tracking-[0.16em] text-slate-500">
              ...
              {availableMenuItems.map(item => (
                <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton
                    ...
                    tooltip={item.label}
                    className="h-10 rounded-lg px-3 text-slate-400 hover:bg-white/[0.07] hover:text-slate-100 data-[active=true]:bg-cyan-300/[0.11] data-[active=true]:text-cyan-200"
                  >
          ...
          <SidebarFooter className="p-3">
            <div className="mb-3 rounded-lg border border-amber-200/10 bg-amber-300/[0.05] px-3 py-2.5 group-data-[collapsible=icon]:hidden">
              <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-200">
                Portfolio preview
              </p>
              <p className="mt-1 text-xs leading-4 text-slate-500">
                Illustrative cases. No live payment data.
              </p>
            </div>
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex w-full items-center gap-3 rounded-lg px-1 py-1 text-left transition-colors hover:bg-white/[0.06]">
                    <Avatar className="h-9 w-9 border border-white/10">
                      <AvatarFallback className="bg-cyan-300/10 text-xs font-semibold text-cyan-200">
        ...
        <div
          className={`absolute right-0 top-0 z-50 h-full w-1 cursor-col-resize transition-colors hover:bg-cyan-300/30 ${isCollapsed ? "hidden" : ""}`}
